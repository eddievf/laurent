...
if(!empty($_SESSION['logged']))
{
	...
	$stmt = $conn->prepare("SELECT CommonName, RestockLevel, Abbrev, Description, inStock, AreaName
							...
							WHERE inStock = 0 
							AND (stockproducts.ReqArea = departamentos.ID)
	...
	echo '
		<form target="_blank" method="POST" id="pdfRequestForm" name="pdfRequestForm">
            <button style="margin-top: 5px; margin-bottom: 15px;" type="submit" value="email" class="btn btn-warning" name="StockPDFEmail" id="StockPDFEmail" onclick="document.pdfRequestForm.action='; echo "'pdf/emailEmpty.php'"; echo '">
                <span class="glyphicon glyphicon-envelope"></span> Enviar Reporte
            </button>
            <button style="margin-top: 5px; margin-bottom: 15px;" type="submit" value="download" class="btn btn-danger" name="ReqPDF" id="ReqPDF"  onclick="document.pdfRequestForm.action='; echo "'pdf/pdfEmptyLog.php'"; echo '">
