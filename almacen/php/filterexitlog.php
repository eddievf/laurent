...
if(!empty($_SESSION['logged']))
{
    ...
    $prod = $_POST['prodExitLog'];
    $fechaInicial = $_POST['dateExitLog'];
    $fechaFinal = $_POST['dateExitLogEnd'];

    $query_string = "SELECT stockproducts.CommonName, WhoRequest, departamentos.AreaName, WhoGives, LogTime, NumMoved, Abbrev, MoveType, RemainStock FROM stocklog, stockproducts, departamentos, produnits WHERE (stocklog.LogProduct = stockproducts.ID) AND (stocklog.WhoArea = departamentos.ID) AND (stocklog.LogUnit = produnits.ID) AND MoveType = 0 ";
	...
	echo '
		<form target="_blank" method="POST" id="pdfRequestForm" name="pdfRequestForm">
            <button style="margin-top: 5px; margin-bottom: 15px;" type="submit" value="email" class="btn btn-warning" name="ExitPDFEmail" id="ExitPDFEmail" onclick="document.pdfRequestForm.action='; echo "'pdf/emailExit.php'"; echo '">
                <span class="glyphicon glyphicon-envelope"></span> Enviar Reporte
            </button>
            <button style="margin-top: 5px; margin-bottom: 15px;" type="submit" value="download" class="btn btn-primary" name="ReqPDF" id="ReqPDF"  onclick="document.pdfRequestForm.action='; echo "'pdf/pdfExitLog.php'"; echo '">
